...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    ...
    // Find the Shopify metaobject sync record
    ...
    if (syncRecord?.shopifyObjectId) {
      // Get publications to find the Online Store publication
      ...
      if (onlineStore) {
        const result = await admin.graphql(PUBLISHABLE_PUBLISH, {
          variables: {
    ...
    console.error("Publish faculty error:", error);
    const message = error instanceof Error ? error.message : "Failed to publish faculty";
